...
exports.getAllClients = (req, res, next) => {
  try{ 
    const db = readDB();
    sendResponse(res, 200, 'clients retrieved successfully', db.client);
    } catch (error) {
    next(error);
  }
};
...
exports.getClientById = (req, res, next) => {
  try {
    const db = readDB();
    const client = db.client.find(c => c.id === parseInt(req.params.id));
    if (!client) {
...
exports.createClient = async(req, res, next) => {
    try {
        ...
        const newClient = {
            id: db.client.length + 1, password: hashedPassword, email, name
        };
...
exports.updateClient = (req, res, next) => {
    try {
        const db = readDB();
        const Index = db.client.findIndex(c => c.id === parseInt(req.params.id));
        if (Index === -1) {
            const error = new Error('Client not found');
            error.status = 404;
            throw error;
        ...
        db.client[Index] = { ...db.client[Index], ...req.body };
        writeDB(db);
        sendResponse(res, 200, 'client updated successfully', db.client[Index]);
    } catch (error) {
        next(error);
    }       
};
